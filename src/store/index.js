...
export default new Vuex.Store({
    ...
    mutations: {
        ...
        editarEnderecos(state, payload) {
            let index = state.enderecos.findIndex(
                endereco => endereco.id == payload.id
            );
        ...
        editaParaTroca(state, payload) {
            ...
            payload[0].troca.forEach(e => {
                let indexPedido = state.pedidos[index].carrinho.findIndex(
                    item => item.cod == e.cod
                );
        ...
        editarCategoria(state, payload) {
            let index = state.categorias.findIndex(
                categoria => categoria.id == payload.id
            );
            Vue.set(state.categorias, index, payload);
        },
        removeCategoria(state, payload) {
            let index = state.categorias.findIndex(
                categoria => categoria.id == payload
            );
